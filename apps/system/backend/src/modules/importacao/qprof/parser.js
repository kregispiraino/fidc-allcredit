...
const columns={cedente:['cedente'],sacado:['sacado'],numero:['snum','titulo'],valor:['vlrpago','valor'],data_liquidacao:['dtaliq','datadeliquidacao'],carteira:['carteira'],carteira_interna:['cartinterna','carteirainterna']};
...
export function parseQprof(buffer){
  let book;
  ...
  const headers=[];
  for(let c=range.s.c;c<=range.e.c;c++)headers[c]=normalize(sheet[XLSX.utils.encode_cell({r:range.s.r,c})]?.v);
  const mapped={};
  ...
  for(let r=range.s.r+1;r<=range.e.r;r++){
    const cells=Object.fromEntries(Object.entries(mapped).map(([key,c])=>[key,sheet[XLSX.utils.encode_cell({r,c})]]));
    ...
    try{
      ...
      row.valor=moneyValue(cells.valor?.v);row.data_liquidacao=dateValue(cells.data_liquidacao?.v,!!book.Workbook?.WBProps?.date1904);
      rows.push(row);
