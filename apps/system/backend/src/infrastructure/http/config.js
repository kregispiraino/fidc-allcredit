...
export function runtimeConfig(env=process.env){
  ...
  return {production,origin,trustProxy:Number(proxy)};
}
export function securityHeaders(config){
  return (req,res,next)=>{
    ...
    if(mutation&&((req.headers.origin&&req.headers.origin!==expected)||req.headers['sec-fetch-site']==='cross-site'))return res.status(403).json({error:'Origem não permitida.'});
    next();
  };
}
